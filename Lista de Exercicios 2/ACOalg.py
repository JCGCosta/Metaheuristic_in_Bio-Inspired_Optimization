...
citiesInfo = pd.read_csv("djibout.csv", delimiter="\t")
distanceTable = pd.read_csv("djiboutDist.csv", delimiter=",", names=["c1", "c2", "dist"])
distanceTable = distanceTable[distanceTable.dist != 0]
numCities = citiesInfo.shape[0]
distanceTable["fero"] = np.ones(distanceTable.shape[0])
...
for g in range(generationMax):
    ...
    print("Best: " + str(bestDistMin[-1]) + " Worst: " + str(bestDistMax[-1]))
    print("BestPath: " + str(pathList[bestDistMin[0] - 1]))
    if 0 < bestDistMax[-1] - bestDistMin[-1] < 1700:
        print("Melhor: " + str(pathList[bestDistMin[0] - 1]))
        break
